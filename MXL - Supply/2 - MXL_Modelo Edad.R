...
### Set core controls
apollo_control = list(
  modelName  ="2 - MXL Modelo Edad",
  modelDescr ="Modelo MXl de Edad",
  mixing=TRUE,
  indivID    ="idLS",
  nCores = 3
)
...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate"){  #esto es una funcion que se crea y al final retorna probabilidades
  ...
  V[['cp']] = (asc_cp + b_Joven*Joven) + b_time * time_cp + b_cost * cost_cp 
...
model = apollo_loadModel("2 - MXL Modelo Edad")

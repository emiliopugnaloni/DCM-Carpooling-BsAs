...
### Set core controls
apollo_control = list(
  modelName  ="2b - MXL Modelo Mujeres Jovenes",
  modelDescr ="Modelo MXl de Mujeres Jovenes (<30)",
  mixing=TRUE,
  indivID    ="idLS",
  nCores = 3
)
...
database$Joven<-0; database$Joven[database$Edad<=30]<-1; database$Adulto<-0;database$Adulto[database$EDAD>30]<-1
database$MujerJoven <-0; database$MujerJoven[database$Edad<30 & database$Mujer==1]<-1
...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate"){  #esto es una funcion que se crea y al final retorna probabilidades
  ...
  V[['cp']] = (asc_cp + b_MujerJoven*MujerJoven) + b_time * time_cp + b_cost * cost_cp 
